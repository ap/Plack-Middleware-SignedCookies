...
test_psgi app => $mw->to_app, client => sub {
	...
	$res = $cb->( GET 'http://localhost/', Cookie => 'cb=1' );
	...
	is $res->content, 'cb=lorem "ipsum";cx=dolor sit amet', 'Own cookies are recognized';

	$res = $cb->( GET 'http://localhost/', Cookie => join '; ', 'cx=1', grep { !/^cx=/ } keys %c );
	is $res->content, 'cb=lorem "ipsum"', 'Tampered cookies are rejected';
